...
#[derive(Default)]
pub struct ReForbiddenFunction {
    re: Option<Regex>,
}
...
impl SyntaxRule for ReForbiddenFunction {
    fn check(
        ...
        if self.re.is_none() {
            self.re = Some(Regex::new(&option.re_forbidden_function).unwrap());
        }
        ...
        match node {
            RefNode::FunctionDeclaration(x) => {
                check_regex(false, unwrap_node!(*x, FunctionIdentifier),
    ...
    fn name(&self) -> String {
        String::from("re_forbidden_function")
    }
