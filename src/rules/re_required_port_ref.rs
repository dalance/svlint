...
impl Rule for ReRequiredPortRef {
    fn check(
        &mut self,
        ...
        if self.re.is_none() {
            self.re = Some(Regex::new(&option.re_required_port_ref).unwrap());
        }
    ...
    fn name(&self) -> String {
        String::from("re_required_port_ref")
    }

    fn hint(&self, option: &ConfigOption) -> String {
        String::from(format!(
            "Use a port identifier matching regex \"{}\".",
            &option.re_required_port_ref
        ))
    }
